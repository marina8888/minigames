...
class Player():
    ...
    def get_value(self, player_deck: list[Card]):
        total_value = 0
    ...
    def hit(self, dealer_deck: list[Card], player_deck: list[Card]):
        player_deck.append(dealer_deck.pop)
        print(player_deck[Card.__str__()])
        return player_deck

    def stay(self, player_deck: list[Card], ace_value=1):
        for card in player_deck:
            if card.name=="Ace":
                card.value=ace_value
            else: print("No Aces in the set")
...
def split(self, player_deck: list[Card]):
    if self.said_split==True
        if player_deck[-1]==player_deck[-2]
            self.number_of_hands+=1
            if self.number_of_hand == 2:
                split_my_cards = []
        ...
        else: print("no identical last cards for splitting")
    else: print("cannot split cards because said no to splitting earlier in game")
